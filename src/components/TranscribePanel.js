...
export default function TranscribePanel({ isHost, socket, roomId, userId, userName, permitted, onClose }) {
  ...
  // Auto-scroll
  useEffect(() => { bottomRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [lines, interimText]);
  ...
  const startRecognition = useCallback(() => {
    ...
    r.onresult = e => {
      ...
      for (let i = e.resultIndex; i < e.results.length; i++) {
        if (e.results[i].isFinal) {
          ...
          const line = { text, speakerName: userName, timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) };
          setLines(p => [...p, line]);
          setInterim('');
    ...
    r.onend   = () => { if (recognRef.current === r) setActive(false); };
    r.start();
    setActive(true);
  ...
  const toggleTranscription = () => { if (active) stopRecognition(); else startRecognition(); };

  const downloadTranscript = () => {
    const text = lines.map(l => `[${l.timestamp}] ${l.speakerName}: ${l.text}`).join('\n');
    const blob = new Blob([text], { type: 'text/plain' });
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob);
    a.download = `transcript-${roomId}.txt`; a.click();
  };

  const clearTranscript = () => { setLines([]); setInterim(''); };
    ...
    <div className={styles.panel}>
      <div className={styles.header}>
        ...
        <div className={styles.headerActions}>
          {lines.length > 0 && <button className={styles.iconBtn} onClick={downloadTranscript} title="Download">⬇</button>}
          {lines.length > 0 && <button className={styles.iconBtn} onClick={clearTranscript} title="Clear">🗑</button>}
        ...
        <>
          <div className={styles.controls}>
            <div className={styles.controlRow}>
              ...
              <select className={styles.select} value={lang} onChange={e => setLang(e.target.value)} disabled={active}>
            ...
            <div className={styles.controlRow}>
              <label className={styles.ctrlLabel}>
                <input type="checkbox" checked={shareToAll} onChange={e => setShare(e.target.checked)} />
                <span style={{marginLeft:6}}>Share with everyone</span>
              </label>
            </div>
            <button className={`${styles.toggleBtn} ${active ? styles.stop : styles.start}`} onClick={toggleTranscription}>
          ...
          <div className={styles.transcript}>
            ...
            {lines.map((l, i) => (
              <div key={i} className={`${styles.line} ${l.remote ? styles.remote : styles.local}`}>
